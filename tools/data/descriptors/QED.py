...
def properties(mol):
    ...
    x = [0] * 8
    x[0] = rdmd._CalcMolWt(mol)
    x[1] = Crippen.MolLogP(mol)
    for hbaPattern in Acceptors:
        if (mol.HasSubstructMatch(hbaPattern)):
            matches = mol.GetSubstructMatches(hbaPattern)
            x[2] += len(matches)
    x[3] = Lipinski.NumHDonors(mol)
    x[4] = MolSurf.TPSA(mol)
    x[5] = Lipinski.NumRotatableBonds(mol)
